...
class Rooms_List extends WP_List_Table
{
	...
	public static function get_records($per_page = 5, $page_number = 1)
	{
		...
		$do_search = ( $search ) ? $wpdb->prepare("WHERE r.description LIKE '%%%s%%'", $search ) : '';

		$table = Plugin_Tables::tables(self::use_table);
		$table_t = Plugin_Tables::tables(self::use_table_t);
		$sql = "SELECT r.*, rt.room_type FROM {$table} r JOIN {$table_t} rt ON r.room_type_id = rt.id {$do_search}";
